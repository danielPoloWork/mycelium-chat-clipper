...
these once, with admin rights, after creating the GitHub repository for `mycelium-chat-clipper`.
...
> and `OWNER=danielPoloWork` / `REPO=mycelium-chat-clipper` exported.

```bash
OWNER=danielPoloWork
REPO=mycelium-chat-clipper
BRANCH=main
```
