...
Full conventions for branches, commits, and pull requests on `mycelium-chat-clipper`. The short
...
- `scope` is a short subsystem noun. Scopes for this repo: `core` `adapters` `extension` `tools` `contract` `e2e` `fixtures` `build` `tests` `docs` `ci` `release` .
