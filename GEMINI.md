...
- **Source layout:** Maven-style cross-language tree. All code under
  `src/main/typescript/it/mycelium-labs/chatclipper/`. Namespace `@mycelium-labs/chatclipper`. See §5.
...
- **Quality bar:** enterprise — warnings-as-errors, `ESLint (typescript-eslint type-aware, eslint-plugin-boundaries for the layer matrix) + tsc --noEmit --strict + commitlint` clean, `Prettier`
  clean, tsc --strict (type soundness); eslint --max-warnings 0; no-network-check (static scan of the production bundle, content script included); size-limit (content script <= 300 kB); pnpm audit; CodeQL green, `consistency_lint.py` passing. No shortcuts. See §10.
